...
###############################################################################
# Date : 2018-5-29
# Author : Xunjunhe & LiKang & ZhangSeyi & FangKun & XinJinquan
# Description : 优化算法
...
class OptimizationDaemon(Daemon):
    ...
    def set_project_info(self, project_info):
        ...
        try:
            ...
            self.optimization_parameter_list = project_info['optimization_parameter']["script"]
            ...
            for list_ in self.optimization_parameter_list:
                min_.append(int(list_['para_quota_min']))

                max_.append(int(list_['para_quota_max']))
                ...
                for l in range(param_count):
                    ...
                    if self.feasible_region(y - const * e_array[l], opt_para_quota_min_list, opt_para_quota_max_list):
                        if (opt_para_expect_list[0] - self.func(y - const * e_array[l], coefficient_array)) ** 2 <= (
                                opt_para_expect_list[0] - self.func(y, coefficient_array)) ** 2:
                            y -= const * e_array[l]
                print(y,self.func(y, coefficient_array),XX[k])
    ...
    def plan_two(self, a_list, PCA_list, opt_para_quota_min_list, opt_para_quota_max_list, opt_para_expect_list):
        ...
        for i, j in zip(PCA_list[0], PCA_list[1]):
            ...
            end_array = np.array(param_list0)
            XX[0] = end_array
            YY[0] = self.func(end_array, coefficient_array)
            const = 1
            for k in range(9):
                y = XX[k]

                for l in range(param_count):
                    if self.feasible_region(y + const * e_array[l], opt_para_quota_min_list,
                                            opt_para_quota_max_list):
                        condition1 = opt_para_expect_list[i] - self.func(y + const * e_array[l], coefficient_array)
                        condition0 = opt_para_expect_list[i] - self.func(y, coefficient_array)
                        if (condition1 ** 2 <= condition0 ** 2) and (condition1 * condition0 > 0):
                            y += const * e_array[l]

                    if self.feasible_region(y - const * e_array[l], opt_para_quota_min_list,
                                            opt_para_quota_max_list):
                        condition2 = opt_para_expect_list[i] - self.func(y - const * e_array[l], coefficient_array)
                        condition0 = opt_para_expect_list[i] - self.func(y, coefficient_array)
                        if (condition2 ** 2 <= condition0 ** 2) and (condition2 * condition0 > 0):
                            y -= const * e_array[l]
                if self.func(XX[k + 1], coefficient_array) == self.func(XX[k], coefficient_array):
                    const *= 0.8
        ...
        for item in ps.listen():
            ...
            while True:
                ...
                if data:
                    ...
                    print("The optimization get the data to be empty!")
                    time.sleep(5)
            ...
            json_body = [
                {
                    ...
                    "fields": {
                        ...
                        "optimization":str(plan_one_dict)
                    }
                }
            ]
